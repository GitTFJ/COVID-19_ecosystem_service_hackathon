...
lcm = raster("data/lcm/lcm2015gb25m.tif")      
crs(lcm) 

#Load local authortiy polygons
la <- st_read("Local_Authority_Districts__December_2017__Boundaries_in_Great_Britain.shp")
crs(la)
class(la)
...
for(b in 1:nrow(crds_comb)){
  ...
  green[
    green == 3 | 
      ...
      green == 20 |
      green == 21 ] <- 0
  green_sum = calculate_lsm(green, level = "landscape")[,5:6]
  green_sum$lad17nm = crds_comb$lad17nm[b]
  green_sum$run = crds_comb$run[b]
  green_cnt = rbind(green_cnt, green_cnt_tmp)
  green_cmb = rbind(green_cmb, green_sum)
}
